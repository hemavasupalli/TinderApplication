export const emailParamsForOTP = (emailId, otp, firstName) => {
  return {
...
export const emailParamsForInterestSent = (toEmailId, senderName) => {
  const htmlBody = `
      <!DOCTYPE html>
      <html>
      <head>
  ...
  return {
    ...
    Message: {
      ...
      Subject: {
        Charset: "UTF-8",
        Data: "Someone is interested in connecting with you ❤️",
      },
    },
    Source: process.env.SES_SOURCE_EMAIL, // verified SES email
  };
};
export const emailParamsForRequestAccepted = (toEmailId, accepterName) => {
  const htmlBody = `
  <!DOCTYPE html>
  <html>
  <head>
